...
import minecraftbyexample.block_my.StartupCommon;
...
public class OreSpawner implements IWorldGenerator {
  ...
  private WorldGenerator diamondSpawner = new WorldGenMinable(
      StartupCommon.blockMy.getDefaultState(), // the block to spawn
      16, // the size of the vein
      BlockMatcher.forBlock(Blocks.STONE)); // the block to replace



  private WorldGenerator bookshelfSpawner = new WorldGenMinable(
          Blocks.BOOKSHELF.getDefaultState(), // the block to spawn
          16, // the size of the vein
  ...
  public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
    ...
    addOreSpawn(diamondSpawner, world, random, chunkX * CHUNK_SIZE, chunkZ * CHUNK_SIZE, 16, 15, 160);


    if(world.provider.getDimension() == 2) {
      addOreSpawn(bookshelfSpawner, world, random, chunkX * CHUNK_SIZE, chunkZ * CHUNK_SIZE, 20, 5, 160);
